...
function ShoppingCart({ visible, toggleCart, cart, addItemToCart, removeItemFromCart, loggedIn, quantities }) {
  ...
  return (
    <div className={`${cartHidden} cart flex flex-col z-20 fixed top-0 right-0 bg-black transition transform ${animation} h-full w-1/3`}>
     <div className="header pl-4">
      ...
      <div className={`items overflow-y-auto h-full p-4`}>
        <div className="p-4"> {/* Optional padding to give some space inside the container */}
          {cart.map(item => (
